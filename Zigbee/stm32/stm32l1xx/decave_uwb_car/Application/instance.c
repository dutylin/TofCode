...
void txCar_list_wait_send(uint8 count)
{
	...
	instance_data_t *inst  =(instance_data_t*) (&instance_data[0]);
	dwt_forcetrxoff() ;
	if(count < 1|| count>Car_revcardlist.u8CardCnt ) //Car_revcardlist.u8CardCnt
		return;
	inst->macdata_msdu[1] = count;             // the number of the send cards
	memcpy(&inst->macdata_msdu[2] ,&my_cardslotmsg.m_distance,2);
	memcpy(&inst->macdata_msdu[4] ,&u16ShortAddr,2);

	for(i=0;i<count;i++)
	{
		memcpy(&inst->macdata_msdu[6+4*i] ,&Car_revcardlist.cardmsg[i].u8cardaddr[0],2);
		inst->macdata_msdu[8+4*i] = Car_revcardlist.cardmsg[i].status;
		inst->macdata_msdu[9+4*i] = Car_revcardlist.cardmsg[i].devtype;
		sendlen = 9+4*i+1;
	}
	
	
	inst->testAppState = TA_CAR_WAIT_SEND ;
	setupmacframedata(inst, sendlen, RTLS_CAR_REVTOFCARD_MSG, !ACK_REQUESTED); //RTLS_MSG_HELP_CALL  RTLS_EXCIT_ASK_SEND
	inst->previousState = TA_CAR_WAIT_SEND ;
	if(instancesendpacket(inst, DWT_START_TX_IMMEDIATE))
	{
	...
	{
//new12		PrintfUtil_vPrintf("txCar send --%d!\n",count);
...
//	mSleep(temp);
	...
	{
//new12		PrintfUtil_vPrintf("send init  --rssi =%i  status =%d \n",inst->i8rssi,inblink_status);
		inst->testAppState = TA_TX_WAIT_CONF ;												 // wait confirmation
		inst->previousState = TA_TXRANGINGINIT_WAIT_SEND ;
		inst->done = INST_DONE_WAIT_FOR_NEXT_EVENT;  //no timeout
	//	dwt_setrxtimeout(0);
...
void set_rssi_dest(uint8 type)
{
	instance_data_t *inst  =(instance_data_t*) (&instance_data[0]);
	if(type)
	{
		//memcpy(&inst->rev_shortaddr16,,ADDR_BYTE_SIZE);
	}
	else
	{
		inst->rev_shortaddr16 = 0xffff;
	}
}
...
void init_car_forrssi()
{
	...
	dwt_setpanid(panid);
	instance_change_channel(ANCHOR_BLINK_CHANNEL);
	dwt_enableframefilter(DISABLE); //allow data, ack frames;
...
//	if(inst->mode == ANCHOR)
	...
	{
		inst->panid = 0xdddd;
		PrintfUtil_vPrintf("CARD	 ADDR = %d \n",u16ShortAddr);
	}
#endif
#ifdef DEC_UWB_SUB
	{
...
//new12				PrintfUtil_vPrintf("revF_sendRep_Y=%d  ",temp-LastrevfinalTick);
				inst->newrange = 1;
				return;
			}
		}
...
void inset_rev_cardmsg(uint16 cardid,uint16 seq, uint8 status, uint8 devtype,int8 rssi)
{
	...
	if(count >= MAX_CAR_CARD_CNT)
		return;
	if(cardid ==0)
		return;
	for(i=0;i<Car_revcardlist.u8CardCnt;i++)
	{
		memcpy(&addr,&Car_revcardlist.cardmsg[i].u8cardaddr[0], 2);
		if(addr == cardid)
			return;
	}
	memcpy(&Car_revcardlist.cardmsg[count].u8cardaddr[0], &cardid,2);
	Car_revcardlist.cardmsg[count].status = abs(rssi);//status ;
	Car_revcardlist.cardmsg[count].devtype =devtype;//  ;
	Car_revcardlist.u8CardCnt++;
	if(count >= MAX_CAR_CARD_SEND_COUNT)
	{
		//event_timer_del(EVENT_CAR_RSSI_SEND);
		event_timer_set(EVENT_CAR_RSSI_SEND);
	}
        ...
        case TA_RX_WAIT_DATA :                                                                     // Wait RX data
            ...
            switch (message)
            {
                ...
                case DWT_SIG_RX_OKAY :
                {
					...
					if(inst->mode == SUB_STA)
					{
						if(inst->i8rssi <-110)  //if the car's card rev the i8rssi is to small, then give up
							return 0;
					}
					switch(fcode)
					{
						...
						{
//new12							PrintfUtil_vPrintf("recv init ---%d\n",srcAddr);
						...
						case RTLS_TDOA_BLINK_SEND:
						{
							...
							PrintfUtil_vPrintf("%d %d %i %d\n",srcAddr, inst->seqnum, inst->i8rssi, systicksPritnf);
					...
					/*	case RTLS_EXCIT_ASK_SEND:
						...
						case RTLS_TOF_MSG_TAG_ACK:
						{
							...
							if(revaddr ==u16ShortAddr)
							{
								...
								instance_set_sleeptick(tick,rev_curslot,slot);
								DBG(PrintfUtil_vPrintf("-in blink !  rev_curslot =%d ,slot=%d ,mysleep=%d  statino=%d\n",rev_curslot,slot,my_cardslotmsg.sleeptick,srcAddr);)
                        ...
                        case RTLS_DEMO_MSG_TAG_FINAL:
                        {
							...
							if(1)//(inst->sendTOFR2Tag)
							{
								...
								if(inst->mode == ANCHOR)
								{
                                    ...
                                    //人卡发送测距报告给主站后可以理解人卡与主站已经完成定位可以在此处置新测距标志
                                    //inst->newrange = 1; 此处不可以置测距结束标志位，此处置位会导致人卡无法和辅站做定位
...
void instance_rxcallback(const dwt_callback_data_t *rxd)
{
    ...
    if(rxd->event == DWT_SIG_RX_OKAY)
    {
    	...
    	if(rxd_event == DWT_SIG_RX_OKAY)
	    {
            ...
            if (instance_data[instance].mode == SUB_STA)
            {
                event_timer_unset(EVENT_RAGING_REPORT);
                event_timer_add(EVENT_RAGING_REPORT, 100);
            }
	    }
...
int instance_run(uint8 type)
{
    ...
    if (gu8CardRestFlag == TRUE)
    {
        gu8CardRestFlag = FALSE;
