...
typedef struct
{
    float  f32TagCoordinateX;
    float  f32TagCoordinateY;
    float  f32TagCoordinateZ;  
}
APP_UWB_TAG_COORDINATE_S;

typedef struct
{
    APP_UWB_TAG_COORDINATE_S stTagCoord;  //基站对应的坐标
    uint_16 u16BsTagDevId;                //目标基站卡号    设备处理需要进行四字节对齐，若不够四字节则会进行补齐后发送
}
APP_UWB_TOF_TAG_COORDINATE_S;

//与其他层级约定好的数据协议结构 协议头 + 数据大小
typedef struct     
{
	APP_HEADER_S stAppHead;
	APP_UWB_TOF_TAG_COORDINATE_S stAppCoordMsg;	
} APP_UWB_TOF_COORDINATE_S;    //放入到协议文件中
